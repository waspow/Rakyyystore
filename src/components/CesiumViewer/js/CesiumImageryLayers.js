...
class meta {
    ...
    static removeExistingLayers(viewer) {
        ...
        for (let i = 0; i < viewer.imageryLayers.length; i++) {
            ...
            if (layer.imageryProvider?.url?.includes('geoserver/Argo' +
                '/wms')) {
                layers.push(layer);
            }
        }
...
class TianWMTSimg {
    static addTianWMTSimg(viewer) {
        const provider = new Cesium.WebMapTileServiceImageryProvider({
            url: "http://t0.tianditu.gov.cn/img_w/wmts?tk=27f2f59170dd2f35fd4e1c91bd290fd7",
            layer: "img",
            style: "default",
            tileMatrixSetID: "w",
            maximumLevel: 18,
        ...
        const annotationProvider = new Cesium.WebMapTileServiceImageryProvider({
            url: "http://t0.tianditu.gov.cn/cia_w/wmts?tk=27f2f59170dd2f35fd4e1c91bd290fd7",
            layer: "cia",
            style: "default",
            tileMatrixSetID: "w",
            maximumLevel: 18,
...
class TianWMTSter {
    static addTianWMTSter(viewer) {
        const provider = new Cesium.WebMapTileServiceImageryProvider({
            url: "http://t0.tianditu.gov.cn/ter_w/wmts?tk=27f2f59170dd2f35fd4e1c91bd290fd7",
            layer: "ter",
            style: "default",
            tileMatrixSetID: "w",
            maximumLevel: 18,
        ...
        const annotationProvider = new Cesium.WebMapTileServiceImageryProvider({
            url: "http://t0.tianditu.gov.cn/cta_w/wmts?tk=27f2f59170dd2f35fd4e1c91bd290fd7",
            layer: "cta",
            style: "default",
            tileMatrixSetID: "w",
            maximumLevel: 18,
...
class TianWMTSvec {
    static addTianWMTSvec(viewer) {
        const provider = new Cesium.WebMapTileServiceImageryProvider({
            url: "http://t0.tianditu.gov.cn/vec_w/wmts?tk=27f2f59170dd2f35fd4e1c91bd290fd7",
            layer: "vec",
            style: "default",
            tileMatrixSetID: "w",
            maximumLevel: 18,
        ...
        const annotationProvider = new Cesium.WebMapTileServiceImageryProvider({
            url: "http://t0.tianditu.gov.cn/vec_w/wmts?tk=27f2f59170dd2f35fd4e1c91bd290fd7", // 替换为你的实际密钥
            layer: "cva",
            style: "default",
            tileMatrixSetID: "w",
            maximumLevel: 18,
    ...
    static initialize(viewer, elementId = 'xyz') {
        ...
        handler.setInputAction(movement => {
            ...
            if (position) {
                ...
                element.innerHTML = `
                    <span style="color: #a3f7bf">纬度：</span>${decimalToDMS(lat, true)}<br>
                    <span style="color: #79d1fb">经度：</span>${decimalToDMS(lon, false)}<br>
                    <span style="color: #ff9a76">高度：</span>${alt} 千米
                `;
...
class WMSsvectorlayer {
    static removeExistingLayers(viewer) {
        ...
        for (let i = 0; i < viewer.imageryLayers.length; i++) {
            const layer = viewer.imageryLayers.get(i);
            if (layer.imageryProvider?.url?.includes('geoserver/Argo/wms')) {
                layers.push(layer);
            }
        }
        layers.reverse().forEach(l => viewer.imageryLayers.remove(l));
...
class CustomWMSBasemap {
    static addArgoBasemap(viewer) {
        // 先移除所有底图
        viewer.imageryLayers.removeAll();
        // 加载 GeoServer WMS 图层
        const provider = new Cesium.WebMapServiceImageryProvider({
            url: "http://localhost:8080/geoserver/Argo/wms",
            layers: "argoproject:argo_temp_2004_01_1975",
            parameters: {
                service: "WMS",
                version: "1.1.0",
                request: "GetMap",
        ...
        viewer.imageryLayers.add(layer);
        // 重新添加meta图层
        meta.addmeta(viewer, store.filterSelections);
    }
}
